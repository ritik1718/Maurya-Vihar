...
// --- Main Add Professor Page Component ---
...
export default function Page() {
  ...
  const validateForm = () => {
    const errors = {};
    // BITS Pilani email validation regex
    const emailRegex = /^[a-zA-Z0-9._%+-]+@pilani\.bits-pilani\.ac\.in$/;
    if (!emailRegex.test(formData.email)) {
        errors.email = "Please enter a valid BITS Pilani email address (@pilani.bits-pilani.ac.in).";
    }

    // Phone number validation regex (must be 10 digits)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to save professor details.');
      }
      
      setMessage({ type: 'success', content: 'New professor has been successfully registered!' });
    ...
    <div className="bg-yellow-50 min-h-screen font-sans text-gray-800 p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <header className="mb-8 text-center">
          <h1 className="text-4xl sm:text-5xl font-bold text-sky-900">Professor Registration</h1>
          <p className="text-lg text-yellow-800 mt-2">Add a new professor to the society's directory.</p>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 rounded-2xl shadow-xl space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label htmlFor="department" className="block text-sm font-medium text-sky-800 mb-1">Department</label>
              <select 
                name="department" 
                id="department" 
                ...
                required 
                className="w-full px-4 py-2 border border-yellow-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition bg-white"
              >
            ...
            <div className="md:col-span-2">
              <label htmlFor="bitsProfileUrl" className="block text-sm font-medium text-sky-800 mb-1">BITS Pilani Profile URL</label>
              <input type="url" name="bitsProfileUrl" id="bitsProfileUrl" value={formData.bitsProfileUrl} onChange={handleInputChange} required placeholder="https://www.bits-pilani.ac.in/pilani/..." className="w-full px-4 py-2 border border-yellow-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition"/>
            </div>
          </div>
